...
if __name__ == "__main__":
    ...
    if '-lamda' in sys.argv:
        p = sys.argv.index('-lamda')
        lamtemp = float(sys.argv[p+1])
        if lamtemp > 0:
            lamda = lamtemp
    ...
    if doOutputFile:
        ...
        for e in range(0,Nexp):
            for t in range(0,Nmeas):
                outfile.write(str(random.Exponential(lamda))+" ")
        ...
        for e in range(0,Nexp):
            for t in range(0,Nmeas):
                print(random.Exponential(lamda), end=' ')
